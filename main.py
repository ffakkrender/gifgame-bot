...
async def cmd_set_nickname(message: Message):
    ...
    if len(parts) < 2 or not parts[1].strip():
        return await message.reply("⚠️ Пример: `ник [жаңа атыңыз]`", parse_mode="Markdown")
    
    new_nick = safe_name(parts[1].strip()[:25])
    user["name"] = new_nick
    user["custom_nick"] = True
    save_data()
    await message.reply(f"✅ Сіздің никнейміңіз **{new_nick}** деп сәтті өзгеретілді!", parse_mode="Markdown")
...
async def start_x50_timer(chat_id):
        ...
        if cat_bets:
            ...
            for b in cat_bets:
                ...
                if is_win:
                    ...
                    result_text += f"💸 **{b['name']}** — ставка {b['stake']:,} → **+{win_amount:,} гиф** ✅\n"
...
async def cb_x50_repeat_bet(cb: CallbackQuery):
    ...
    last = x50_last_bets[uid]
    stake = last["stake"]
    norm_code = last["choice"]
    
    if user["balance"] < stake:
        return await cb.answer(f"⚠️ Недостаточно средств! Баланс: {user['balance']:,} гиф", show_alert=True)
    ...
    quote_text = (
        f"🔁 **{user['name']}** повторил ставки в игре **Х50** на сумму **{stake:,} гиф**:\n\n"
        f"> **{stake:,}** на {color_emo} **{mult_str}**"
    )
...
async def process_help_callback(cb: CallbackQuery):
    ...
    if action == "games":
        ...
        text = (
            ...
            "• `банк` — проверить банк\n"
            "• `ник [аты]` — атыңызды/никнейміңізді өзгерту\n"
...
async def cmd_start(message: Message):
    ...
    await message.reply(
        "🎰 **БОТ КАЗИНО**\n\n"
        "• `баланс` | `банк` | `профиль` | `ник [аты]` | `топ` | `лб` | `бонус`\n"
